...
# IBP DS — правила для opencode

Этот файл читает **opencode**. Он намеренно короткий: подробные правила вынесены
в `.opencode/rules/ds-rules.md` (подключается через `instructions` в `opencode.json`).
...
в `styles/*.css`, классы компонентов и страницы документации в `pages/`.
...
Результат сборки — `pages/screens/genOD/<Имя>.html` +
`pages/screens/genOD/<Имя>.screen.md`.

## Макет живой

Экран подключает `scripts/ds.js` — все рантаймы ДС работают, если в разметке
стоят их хуки (`data-table`, `data-modal`, `data-tabs`, `data-popover`,
