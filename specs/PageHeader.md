...
## Назначение
PageHeader — заголовок страницы рабочей области: идентифицирует открытую сущность (сделка, инструмент, раздел) и несёт основные действия страницы. Состоит из независимых частей: Title (+иконка слева, +кнопка редактирования), Chips, кнопка возврата (Return), Subtitle, Actions (+MenuButton). Адаптируется вплоть до мобильной раскладки. Breadcrumbs — отдельный компонент, ставится строкой НАД PageHeader.
...
- **Анатомия** — `.phead` = `.phead__main` (колонка: `.phead__title-row` → `.phead__subtitle`) + `.phead__actions`. В title-row: `.phead__title-ico` (опц.) · `.phead__title` · `.phead__edit` (IconButton S, опц.) · `.phead__chips` (опц.) · `.phead__return` (опц.).
- **Варианты** — все части независимо опциональны (16 комбинаций Subtitle×Chips×Return×Actions); Dashboard=Yes → `.phead--dashboard` (подложка `--bgtable-pinned`, радиус 8, паддинг 12/16).
- **Title** — токен `--type-h3-strong` (28/32), усечение в одну строку + Tooltip. Иконка слева 24px `--text-inactive` — информационная. Edit — IconButton neutral S (глиф `edit`), запускает переименование сущности.
...
- **Subtitle** — Body S `--text-secondary`, опц. иконка 16 слева/справа (`--success`). Кастомный вариант — встроенные мета-элементы `.phead__meta` (Body XS `--text-inactive`, иконка+текст), например «7 · Версия 12 (07.02.2021)».
...
- **Поведение** — адаптивность: <1024 второстепенные actions уходят в MenuButton; <720 — `.phead--stack` (actions вниз, на всю ширину; Title переносится максимум в 2 строки). Усечение Title → Tooltip с полным значением. Переполнение chips — чип-счётчик «+N».
...
- **Цвета** — Title `--text-primary`; иконка слева и мета `--text-inactive`; Subtitle `--text-secondary`; иконка сабтайтла/мета-ок `--success`; подложка Dashboard `--bgtable-pinned`.
...
| `.phead__title-row` | строка частей заголовка, gap 8, min-height 32 |
| `.phead__title-ico` | иконка слева 24, `--text-inactive` |
| `.phead__title` | h1, `--type-h3-strong`, усечение ellipsis |
...
| `.phead--dashboard` | подложка `--bgtable-pinned`, радиус 8, паддинг 12/16 |
