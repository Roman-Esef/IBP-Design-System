...
- **Состав** — Bare (только трек) / Заголовок+Значение (базовый вариант формы) / +Хелпер / Плавающее значение (метка следует за краем заливки — второй исходный референс, «Вероятность сделки») / Компактный (без вертикальных зазоров, для таблиц/списков — добавлено).
