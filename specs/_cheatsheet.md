...
**Классы:** `.dtable` · `.dtable__toolbar` / `-left` / `-right` · `.dtable__title` / `__count` · `.dtable__body` · `.dtable__edge--l` / `--r` · `.dtable--scrolled` · `.dtable--edge-l` / `--edge-r` · `.dtable__footer` · `.dtable__empty` / `-title` / `-text` · `aria-busy="true"` · `.tbl` / `.tbl__row` / `.th` / `.tc` · `.pgn-row` / `.pgn-bulk`
...
**Из коробки:** подключить `scripts/ds-table.js` — самоинициализация по `.dtable__body` (scroll + ResizeObserver), выставляет `.dtable--scrolled`/`--edge-l`/`--edge-r`. Императивно на динамически вставленный узел — `DSTable.bind(bodyEl)` или `DSTable.bindAll(root)`. Колонки (resize/reorder/pin/sort), выбор строк и bulk-режим — модель данных на стороне потребителя, из коробки не даём. Отдельная колонка «Действие» — кнопки видны всегда (не по hover, в отличие от `.tc__hidden`); действие массового удаления в `.pgn-bulk` обязательно дублируется на уровне строки в этой колонке.
