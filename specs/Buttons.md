...
- **Type** — Свойство Type определяет общий вид кнопки: Accent, Outline, Transparent.
- **Danger** — Модификатор (не отдельный тип), комбинируется с Accent/Outline/Transparent, меняет цвет на токены Error. Для деструктивного действия в диалоге подтверждения (см. Modal) — не для обычного «Удалить» в подвале формы, которое остаётся Transparent.
- **Кнопка с меню (chevron)** — Если кнопка содержит шеврон — она открывает контекстное меню. Клик по самой кнопке только показывает меню; действия выполняются исключительно по пунктам меню. Справа может быть либо шеврон, либо иконка — но не одновременно. Нажмите, чтобы открыть.
...
- **Отступы** — Отступы внутри кнопок разного типа, но одного состава — идентичны. Все значения в пикселях.
...
- **Fullwidth** — Кнопки со свойством Fullwidth заполняют весь контейнер: боковые паддинги убираются, контент центрируется. Кнопки, содержащие только иконку, не растягиваются.
- **Состояния** — Default, Hover, Active, Disabled — для каждого типа.
- **Loader** — Сообщает, что идёт процесс, вызванный кликом. На время процесса кнопка переходит в Disabled. Если в кнопке есть иконка — лоадер заменяет её (при иконках с обеих сторон заменяется левая). Если иконок нет — лоадер появляется слева от текста и увеличивает ширину. В IconOnly иконка заменяется лоадером. Нажмите на любую кнопку, чтобы увидеть.
