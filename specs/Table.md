...
deps: [table-cell, pagination, table-filter, button, icon-button, chip, checkbox, illustration]
status: curated
---
...
- **Варианты** — Toolbar включаемый; липкая шапка не отключаема (пока есть вертикальный скролл тела); массовые действия — часть Footer, требуют его включения; горизонтальный скролл + закрепление колонок — свойство TableCell, Table добавляет только тени по краям.
