...
deps: [checkbox, chip, icon-button, button, input, dropdown-list, tooltip]
status: curated
---
...
- **Поведение · ширина колонки** — ручка на правой границе шапки (`.th__resize`, зона захвата 9px, графика только по hover/focus); drag пересчитывает ширину вживую, минимум **96px**, клавиатура ← → шагом 16px. Свободное место справа забирает замыкающий разделитель (`grid: 8px … minmax(8px,1fr)`).
...
<div class="tc tc--tree" style="--tc-level:1"><button class="tc__twisty" aria-expanded="true"><svg chevron/></button><span class="tc__row"><span class="tc__text">Дочернее ООО</span></span></div>
<div class="tc tc--center"><label class="cb cb--no-content cb--selected"><input type="checkbox" class="cb__input" checked><span class="cb__box"><span class="cb__mark"><svg check/></span></span></label></div>
<div class="tc"><div class="tc__hidden"><button class="ibtn ibtn--neutral ibtn--s" aria-label="Изменить"><svg…/></button></div></div>
<div class="tc tc--input tc--edited"><div class="inp inp--s inp--fullwidth"><div class="inp__field"><input class="inp__control" value="12.05.2026"><span class="inp__acts"><button class="inp__act" aria-label="Открыть календарь"><svg calendar/></button></span></div></div></div>
...
<div class="th th--sorted th--pinned" aria-sort="descending">
  ...
  <span class="th__tools">
    <button class="ibtn ibtn--neutral ibtn--s th__pin" aria-pressed="true" aria-label="Открепить колонку"><svg pin-filled/></button>
    <button class="ibtn ibtn--neutral ibtn--s th__sort" aria-label="Сортировка от большего к меньшему"><svg arrow-narrow-down/></button>
  </span>
</div>
```
...
- **Ширина колонки**: drag правой границы шапки ⇢ `width = max(96, старт + dx)`, линия-указатель через всю таблицу; клавиатура ← → шагом 16px.
