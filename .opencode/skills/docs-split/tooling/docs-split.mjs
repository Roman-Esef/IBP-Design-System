...
const INDEX_MD = path.join(ROOT, 'specs', '_index.md');
...
async function resolveCss(pagePath, explicitCss) {
  ...
  if (cssMap[name]) return path.join(ROOT, cssMap[name]);
  // fallback: kebab + нижний регистр (snackbar, riskmetric — без дефиса)
  const cands = [kebab(name) + '.css', name.toLowerCase() + '.css'];
  for (const c of cands) {
    const p = path.join(ROOT, 'styles', c);
    if (existsSync(p)) return p;
  }
  fail(`не найден CSS для ${name} — укажи явно: --css styles/<файл>.css`);
}
...
async function cmdMap() {
  ...
  for (const dir of PAGE_DIRS) {
    const dirPath = path.join(ROOT, 'pages', dir);
