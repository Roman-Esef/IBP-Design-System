...
purpose: Раскатка страниц документации IBP (pages/**) на паттерн «сплиттер + табы» (docs-split) — процедура, тулчейн, правила чтения
checked: "30.08.2026 — пилот AllocationBar2, тулчейн check/inject/verify/map работает"
---

# Раскатка `pages/**` на docs-split

## Когда применять

Поручение касается страниц документации (`pages/foundations|atoms|molecules|organisms/**`).
...
`styles/docs-split.css` + `scripts/docs-split.js` (не трогать).
...
3. **CSS не читать и не перепечатывать.** Вкладку «Код» заполняет
   ...
   `styles/<компонент>.css` в `src-code-css` из файла. Модель CSS не видит.
...
```
node .opencode/skills/docs-split/tooling/docs-split.mjs <cmd> [page] [--css styles/x.css]
```

| Команда | Что делает |
|---|---|
| `map` | перегенерирует `references/pages-index.md` (структурная карта всех doc-страниц) |
| `inject <page>` | вставляет полный CSS компонента в `src-code-css` (маппинг имя→css из `specs/_index.md`; для страниц без спеки — `--css`) |
...
- `styles/`, `scripts/`, `specs/`, `pages/**` (кроме раскатываемой страницы) —
